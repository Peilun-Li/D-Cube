...
for file in glob.glob("*.csv"):
    ...
    try:
        ...
        elif 'parameters' in relation:
            drop_and_create_table(db_conn, relation, "block varchar(20), density varchar(20), row_count varchar(20), time varchar(20)")
            exec_sql(db_conn, "copy %s(block, density, row_count, time) from '%s' delimiter ',' CSV HEADER" % (relation, path))
            # exec_sql(db_conn, "alter table %s alter column block type integer using block::integer" % relation)
            # exec_sql(db_conn, "alter table %s alter column density type numeric using density::numeric" % relation)
            # exec_sql(db_conn, "alter table %s alter column row_count type integer using row_count::integer" % relation)
            # exec_sql(db_conn, "alter table %s alter column time type numeric using time::numeric" % relation)
